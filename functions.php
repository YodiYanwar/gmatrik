<?php 
    ...
    for ($i = 1; $i < $count; $i++) {
		...
		//hasil $out
		$query = 'INSERT INTO pekan(pekan, id_semester, tanggal_dari, tanggal_sampai) VALUES ';
		$query_parts = array();
		$pekan_n = 1;
    for($x=0; $x<count($out); $x++){
				$query_parts[] = "(" . $pekan_n . ", ((SELECT s.id_semester FROM semester s WHERE '".date('Y-m-d', $out[$x]["start"])."' BETWEEN s.tanggal_dari AND s.tanggal_sampai)), '" . date('Y-m-d', $out[$x]["start"]) . "', '" . date('Y-m-d', $out[$x]["end"]) . "')";
				$pekan_n++;
    }
    ...
    for ($i = 1; $i < $count; $i++) {
		...
		$pekan_n = 1;
    for($x=0; $x<count($out); $x++){
				$query = "INSERT INTO pekan(pekan, id_semester, tanggal_dari, tanggal_sampai) VALUES ('" . $pekan_n . "', (SELECT s.id_semester FROM semester s WHERE '".$timeStart."' BETWEEN s.tanggal_dari AND s.tanggal_sampai), '" . date('Y-m-d', $out[$x]["start"]) . "', '" . date('Y-m-d', $out[$x]["end"]) . "')";
				mysql_query($query);
				$pekan_n++;
    }
		// $query .= implode(',', $query_parts);
	...
	// function tampilPekanTalim(){
	// 	$ambildata = mysql_query("SELECT p.id_semester, s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_sampai FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan FROM presensi_talim ps LEFT JOIN talim t ON ps.id_talim = t.id_talim GROUP BY t.id_pekan ) s ON p.id_pekan = s.id_pekan ORDER BY p.id_pekan DESC") or die(mysql_error());
	// 	if (mysql_num_rows($ambildata) > 0) {
	// 		while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
	// 			$data[] = $ad;
	// 			return $data;
	...
	function tampilPekanTalim(){
		$ambildata = mysql_query("SELECT s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_dari, p.tanggal_sampai, t.total FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan, COUNT(t.id_talim) AS total FROM talim t GROUP BY t.id_pekan ) t ON p.id_pekan = t.id_pekan ORDER BY p.id_pekan") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilPekanTalimByPembina($idPembina){
		$ambildata = mysql_query("SELECT s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_dari, p.tanggal_sampai, t.total FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan, COUNT(t.id_talim) AS total FROM talim t WHERE t.id_pembina = $idPembina GROUP BY t.id_pekan ) t ON p.id_pekan = t.id_pekan ORDER BY p.id_pekan") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
			...
			echo "<div class='alert alert-warning alert-dismissibl' role='alert'>
							...
							Tidak ada data pekan presensi talim karena anda belum menambah data talim & input presensi talim
						</div>";
		}
	}

	function tampilPekanTalimByPembinaRoleMhs($nim){
		$ambildata = mysql_query("SELECT s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_dari, p.tanggal_sampai, t.total FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan, COUNT(t.id_talim) AS total FROM talim t WHERE t.id_pembina = (SELECT m.id_pembina FROM mahasiswa m WHERE m.nim = $nim) GROUP BY t.id_pekan ) t ON p.id_pekan = t.id_pekan ORDER BY p.id_pekan") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilPekanTahsin(){
		$ambildata = mysql_query("SELECT s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_sampai, t.total FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan, COUNT(t.id_tahsin) AS total FROM tahsin t GROUP BY t.id_pekan ) t ON p.id_pekan = t.id_pekan ORDER BY p.id_pekan") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilPekanTahsinByPembina($idPembina){
		$ambildata = mysql_query("SELECT s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_sampai, t.total FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan, COUNT(t.id_tahsin) AS total FROM tahsin t WHERE t.id_pembina = $idPembina GROUP BY t.id_pekan ) t ON p.id_pekan = t.id_pekan ORDER BY p.id_pekan") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
			...
			echo "<div class='alert alert-warning alert-dismissibl' role='alert'>
							<button type='button' class='close' data-dismiss='alert' aria-label='Close'></button>
							Tidak ada data pekan presensi tahsin/tahfidz karena anda belum menambah data tahsin/tahfidz & input presensi tahsin/tahfidz
						</div>";
		}
	}			

	function tampilPekanTahsinRoleMhs($nim){
		$ambildata = mysql_query("SELECT s.semester, p.id_pekan, p.pekan, p.tanggal_dari, p.tanggal_sampai, t.total FROM pekan p LEFT JOIN semester s ON p.id_semester = s.id_semester INNER JOIN ( SELECT t.id_pekan, COUNT(t.id_tahsin) AS total FROM tahsin t WHERE t.id_pembina = (SELECT m.id_pembina FROM mahasiswa m WHERE m.nim = $nim) GROUP BY t.id_pekan ) t ON p.id_pekan = t.id_pekan ORDER BY p.id_pekan") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiShalatByPekan($idPekan){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, m.aktif , p.nama AS namapembina, p.gelar, IF(s.total IS NULL, 0, s.total) AS total, 35 AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(g.jplg IS NULL, 0, g.jplg) AS jplg, 35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg)) AS target2, IF(ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2) > 100, 100, ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2)) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT ps.nim, SUM(ps.shubuh+ps.dzuhur+ps.ashar+ps.maghrib+ps.isya) AS total FROM presensi_shalat ps WHERE ps.id_pekan = $idPekan GROUP BY ps.nim ) s ON m.nim = s.nim LEFT JOIN ( SELECT us.nim, SUM(us.shubuh+us.dzuhur+us.ashar+us.maghrib+us.isya) AS jmlu FROM udzur_shalat us WHERE us.disetujui = 1 AND us.id_pekan = $idPekan GROUP BY us.nim ) u ON m.nim = u.nim LEFT JOIN ( SELECT jp.gender, SUM(jp.shubuh+jp.dzuhur+jp.ashar+jp.maghrib+jp.isya) AS jplg FROM jadwal_pulang jp WHERE jp.id_pekan = $idPekan GROUP BY jp.gender ) g ON m.gender = g.gender WHERE m.aktif = 1 ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiShalatByPekanByPembina($idPekan, $idPembina){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, m.aktif, p.nama AS namapembina, p.gelar, IF(s.total IS NULL, 0, s.total) AS total, 35 AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(g.jplg IS NULL, 0, g.jplg) AS jplg, 35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg)) AS target2, IF(ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2) > 100, 100, ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2)) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT ps.nim, SUM(ps.shubuh+ps.dzuhur+ps.ashar+ps.maghrib+ps.isya) AS total FROM presensi_shalat ps WHERE ps.id_pekan = $idPekan GROUP BY ps.nim ) s ON m.nim = s.nim LEFT JOIN ( SELECT us.nim, SUM(us.shubuh+us.dzuhur+us.ashar+us.maghrib+us.isya) AS jmlu FROM udzur_shalat us WHERE us.disetujui = 1 AND us.id_pekan = $idPekan GROUP BY us.nim ) u ON m.nim = u.nim LEFT JOIN ( SELECT jp.gender, SUM(jp.shubuh+jp.dzuhur+jp.ashar+jp.maghrib+jp.isya) AS jplg FROM jadwal_pulang jp WHERE jp.id_pekan = $idPekan GROUP BY jp.gender ) g ON m.gender = g.gender WHERE m.id_pembina = $idPembina AND m.aktif = 1 ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiShalatByPekanByMhs($idPekan, $nim){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, IF(s.total IS NULL, 0, s.total) AS total, 35 AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(g.jplg IS NULL, 0, g.jplg) AS jplg, 35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg)) AS target2, IF(ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2) > 100, 100, ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2)) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT ps.nim, SUM(ps.shubuh+ps.dzuhur+ps.ashar+ps.maghrib+ps.isya) AS total FROM presensi_shalat ps WHERE ps.id_pekan = $idPekan GROUP BY ps.nim ) s ON m.nim = s.nim LEFT JOIN ( SELECT us.nim, SUM(us.shubuh+us.dzuhur+us.ashar+us.maghrib+us.isya) AS jmlu FROM udzur_shalat us WHERE us.disetujui = 1 AND us.id_pekan = $idPekan GROUP BY us.nim ) u ON m.nim = u.nim LEFT JOIN ( SELECT jp.gender, SUM(jp.shubuh+jp.dzuhur+jp.ashar+jp.maghrib+jp.isya) AS jplg FROM jadwal_pulang jp WHERE jp.id_pekan = $idPekan GROUP BY jp.gender ) g ON m.gender = g.gender WHERE m.nim = $nim ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTalimByPekan($idPekan){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, p.gelar, IF(ta.total IS NULL, 0, ta.total) AS total, IF(tl.target IS NULL, 0, tl.target) AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu) AS target2, IF(ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) > 100, 100, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2)) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_talim pt LEFT JOIN talim t ON pt.id_talim = t.id_talim WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM talim t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_talim ut LEFT JOIN talim t ON ut.id_talim = t.id_talim WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTalimByPekanByPembina($idPekan, $idPembina){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, IF(ta.total IS NULL, 0, ta.total) AS total, IF(tl.target IS NULL, 0, tl.target) AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu) AS target2, IF(ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) > 100, 100, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2)) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_talim pt LEFT JOIN talim t ON pt.id_talim = t.id_talim WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM talim t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_talim ut LEFT JOIN talim t ON ut.id_talim = t.id_talim WHERE ut.disetujui = 1 AND t.id_pekan = $idPekan GROUP BY ut.nim ) u ON m.nim = u.nim WHERE m.id_pembina = $idPembina ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTalimByPekanByMhs($idPekan, $nim){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, IF(ta.total IS NULL, 0, ta.total) AS total, IF(tl.target IS NULL, 0, tl.target) AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu) AS target2, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_talim pt LEFT JOIN talim t ON pt.id_talim = t.id_talim WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM talim t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_talim ut LEFT JOIN talim t ON ut.id_talim = t.id_talim WHERE ut.disetujui = 1 AND t.id_pekan = $idPekan GROUP BY ut.nim ) u ON m.nim = u.nim WHERE m.nim = $nim ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTahsinByPekanByPembina($idPekan, $idPembina){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, IF(ta.total IS NULL, 0, ta.total) AS total, IF(tl.target IS NULL, 0, tl.target) AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu) AS target2, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_tahsin pt LEFT JOIN tahsin t ON pt.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM tahsin t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_tahsin ut LEFT JOIN tahsin t ON ut.id_tahsin = t.id_tahsin WHERE ut.disetujui = 1 AND t.id_pekan = $idPekan GROUP BY ut.nim ) u ON m.nim = u.nim WHERE m.id_pembina = $idPembina ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTahsinByPekanByMhs($idPekan, $nim){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama, p.gelar, IF(ta.total IS NULL, 0, ta.total) AS total, IF(tl.target IS NULL, 0, tl.target) AS target1, IF(u.jmlu IS NULL, 0, u.jmlu) AS jmlu, IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu) AS target2, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_tahsin pt LEFT JOIN tahsin t ON pt.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM tahsin t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_tahsin ut LEFT JOIN tahsin t ON ut.id_tahsin = t.id_tahsin WHERE ut.disetujui = 1 AND t.id_pekan = $idPekan GROUP BY ut.nim ) u ON m.nim = u.nim WHERE m.nim = $nim ORDER BY m.nama") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTotalnByPekan($idPekan){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, p.gelar, IF(s.nilai = 0, NULL, s.nilai) AS shalat, tl.nilai AS talim, th.nilai AS tahsin, ROUND((((IF(s.nilai = 0, NULL, s.nilai))*(65/100))+(tl.nilai*(15/100))+(th.nilai*(20/100))),2) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT m.nim, ROUND((IF(ROUND(((s.total/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2) > 100, 100, ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2))),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT ps.nim, SUM(ps.shubuh+ps.dzuhur+ps.ashar+ps.maghrib+ps.isya) AS total FROM presensi_shalat ps WHERE ps.id_pekan = $idPekan GROUP BY ps.nim ) s ON m.nim = s.nim LEFT JOIN ( SELECT us.nim, SUM(us.shubuh+us.dzuhur+us.ashar+us.maghrib+us.isya) AS jmlu FROM udzur_shalat us WHERE us.disetujui = 1 AND us.id_pekan = $idPekan GROUP BY us.nim ) u ON m.nim = u.nim LEFT JOIN ( SELECT jp.gender, SUM(jp.shubuh+jp.dzuhur+jp.ashar+jp.maghrib+jp.isya) AS jplg FROM jadwal_pulang jp WHERE jp.id_pekan = $idPekan GROUP BY jp.gender ) g ON m.gender = g.gender ) s ON m.nim = s.nim LEFT JOIN ( SELECT m.nim, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_talim pt LEFT JOIN talim t ON pt.id_talim = t.id_talim WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM talim t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_talim ut LEFT JOIN talim t ON ut.id_talim = t.id_talim WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ) tl ON m.nim = tl.nim LEFT JOIN ( SELECT m.nim, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_tahsin pt LEFT JOIN tahsin t ON pt.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM tahsin t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_tahsin ut LEFT JOIN tahsin t ON ut.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ) th ON m.nim = th.nim ORDER BY m.nama ") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTotalnByPekanByPembina($idPekan, $idPembina){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, p.gelar, IF(s.nilai = 0, NULL, s.nilai) AS shalat, tl.nilai AS talim, th.nilai AS tahsin, ROUND((((IF(s.nilai = 0, NULL, s.nilai))*(65/100))+(tl.nilai*(15/100))+(th.nilai*(20/100))),2) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT m.nim, ROUND((IF(ROUND(((s.total/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2) > 100, 100, ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2))),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT ps.nim, SUM(ps.shubuh+ps.dzuhur+ps.ashar+ps.maghrib+ps.isya) AS total FROM presensi_shalat ps WHERE ps.id_pekan = $idPekan GROUP BY ps.nim ) s ON m.nim = s.nim LEFT JOIN ( SELECT us.nim, SUM(us.shubuh+us.dzuhur+us.ashar+us.maghrib+us.isya) AS jmlu FROM udzur_shalat us WHERE us.disetujui = 1 AND us.id_pekan = $idPekan GROUP BY us.nim ) u ON m.nim = u.nim LEFT JOIN ( SELECT jp.gender, SUM(jp.shubuh+jp.dzuhur+jp.ashar+jp.maghrib+jp.isya) AS jplg FROM jadwal_pulang jp WHERE jp.id_pekan = $idPekan GROUP BY jp.gender ) g ON m.gender = g.gender ) s ON m.nim = s.nim LEFT JOIN ( SELECT m.nim, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_talim pt LEFT JOIN talim t ON pt.id_talim = t.id_talim WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM talim t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_talim ut LEFT JOIN talim t ON ut.id_talim = t.id_talim WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ) tl ON m.nim = tl.nim LEFT JOIN ( SELECT m.nim, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_tahsin pt LEFT JOIN tahsin t ON pt.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM tahsin t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_tahsin ut LEFT JOIN tahsin t ON ut.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ) th ON m.nim = th.nim WHERE m.id_pembina = $idPembina ORDER BY m.nama ") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
	...
	function tampilNilaiTotalnByPekanByMhs($idPekan, $nim){
		$ambildata = mysql_query("SELECT m.nim, m.nama, m.gender, m.id_pembina, p.nama AS namapembina, p.gelar, IF(s.nilai = 0, NULL, s.nilai) AS shalat, tl.nilai AS talim, th.nilai AS tahsin, ROUND((((IF(s.nilai = 0, NULL, s.nilai))*(65/100))+(tl.nilai*(15/100))+(th.nilai*(20/100))),2) AS nilai FROM mahasiswa m INNER JOIN pembina_mahasiswa p ON m.id_pembina = p.id_pembina LEFT JOIN ( SELECT m.nim, ROUND((IF(ROUND(((s.total/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2) > 100, 100, ROUND(((IF(s.total IS NULL, 0, s.total)/(35-IF(u.jmlu IS NULL, 0, u.jmlu)-(IF(g.jplg IS NULL, 0, g.jplg))))*100),2))),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT ps.nim, SUM(ps.shubuh+ps.dzuhur+ps.ashar+ps.maghrib+ps.isya) AS total FROM presensi_shalat ps WHERE ps.id_pekan = $idPekan GROUP BY ps.nim ) s ON m.nim = s.nim LEFT JOIN ( SELECT us.nim, SUM(us.shubuh+us.dzuhur+us.ashar+us.maghrib+us.isya) AS jmlu FROM udzur_shalat us WHERE us.disetujui = 1 AND us.id_pekan = $idPekan GROUP BY us.nim ) u ON m.nim = u.nim LEFT JOIN ( SELECT jp.gender, SUM(jp.shubuh+jp.dzuhur+jp.ashar+jp.maghrib+jp.isya) AS jplg FROM jadwal_pulang jp WHERE jp.id_pekan = $idPekan GROUP BY jp.gender ) g ON m.gender = g.gender ) s ON m.nim = s.nim LEFT JOIN ( SELECT m.nim, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_talim pt LEFT JOIN talim t ON pt.id_talim = t.id_talim WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM talim t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_talim ut LEFT JOIN talim t ON ut.id_talim = t.id_talim WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ) tl ON m.nim = tl.nim LEFT JOIN ( SELECT m.nim, ROUND(((IF(ta.total IS NULL, 0, ta.total)/(IF(tl.target IS NULL, 0, tl.target)-IF(u.jmlu IS NULL, 0, u.jmlu)))*100),2) AS nilai FROM mahasiswa m LEFT JOIN ( SELECT pt.nim, COUNT(pt.nim) AS total FROM presensi_tahsin pt LEFT JOIN tahsin t ON pt.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan GROUP BY pt.nim ) ta ON m.nim = ta.nim LEFT JOIN ( SELECT t.id_pembina, COUNT(t.id_pembina) AS target FROM tahsin t WHERE t.id_pekan = $idPekan GROUP BY t.id_pembina ) tl ON m.id_pembina = tl.id_pembina LEFT JOIN ( SELECT ut.nim, COUNT(ut.id_udzur) AS jmlu FROM udzur_tahsin ut LEFT JOIN tahsin t ON ut.id_tahsin = t.id_tahsin WHERE t.id_pekan = $idPekan AND ut.disetujui = 1 GROUP BY ut.nim ) u ON m.nim = u.nim ) th ON m.nim = th.nim WHERE m.nim = $nim ORDER BY m.nama ") or die(mysql_error());
		if (mysql_num_rows($ambildata) > 0) {
			while ($ad = mysql_fetch_assoc($ambildata)) // Perulangan while ini JANGAN pake {}
				$data[] = $ad;
				return $data;
...
/*	function tambahSemester($angkatan, $semester, $tanggaldari, $tanggalsampai){
	...
	function importShalat2($angkatan, $from, $to, $_shubuhFrom, $_shubuhTo, $_dzuhurFrom, $_dzuhurTo, $_asharFrom, $_asharTo, $_maghribFrom, $_maghribTo, $_isyaFrom, $_isyaTo){
		...
		while($row_mdb = odbc_fetch_array($result)){
			...
			//if(strpos($row_mdb['Name'], $row_mdb['Badgenumber']) === FALSE){
			$mysql_insert_presensi = "INSERT INTO presensi_shalat(nim, id_pekan, tanggal, shubuh, dzuhur, ashar, maghrib, isya) VALUES 
			(".$row_mdb['nim'].", ((SELECT p.id_pekan FROM pekan p WHERE '$tgl_' BETWEEN p.tanggal_dari AND p.tanggal_sampai)), '$tgl_', '".$shubuh_."','".$dzuhur_."','".$ashar_."','".$maghrib_."','".$isya_."');";
